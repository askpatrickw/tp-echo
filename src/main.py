...
spi = machine.SPI(
    sck=machine.Pin(TP.DOTSTAR_CLK),
...
GetBinaryState_soap = """<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
    ...
    <u:GetBinaryStateResponse xmlns:u="urn:Belkin:service:basicevent:1">
        <BinaryState>%(state_relay)s</BinaryState>
...
class poller:
    ...
    def poll(self, timeout=100):
        ...
        for one_ready in ready:
            ...
            if target:
                # dbg(
                #     "get socket with fileno: %s" % str(one_ready[0].fileno()) +
                #     " len: %s" % len(one_ready) +
                #     " selected: %s " % str(target.fileno())
                #     )
                # update time
...
class upnp_device:
    ...
    def respond_to_search(self, destination, search_target):
        ...
        date_str = format_timetuple_and_zone(CLOCK.datetime(), "GMT")
...
class fauxmo(upnp_device):
    ...
    def handle_request(self, data, sender, socket):
        if (
            ...
            state = self.getState()
            soap = GetBinaryState_soap % {"state_relay": state}
            date_str = format_timetuple_and_zone(CLOCK.datetime(), "GMT")
            message = (
                "HTTP/1.1 200 OK\r\n"
                "CONTENT-LENGTH: %d\r\n"
...
def thread_echo(args):
    ...
    """
        ...
        # handler object with 'on' and 'off' methods (e.g. rest_api_handler(COLOR)})
        # port #
    ...
    devices = [
        {
            "description": "Green L.E.D.",
            "port": 12340,
            "handler": rest_api_handler(GREEN),
        },
        {
            "description": "Red L.E.D.",
            "port": 12341,
            "handler": rest_api_handler(RED),
        },
        {
            "description": "Blue L.E.D.",
            "port": 12342,
            "handler": rest_api_handler(BLUE),
        }
    ]
    ...
    # setting the system time using ntp
    import ntptime
    ntptime.settime()
